...
A complementary, optional path is the [dev-time hook](../how-to/dev-time-hook.md) — a local pre-commit hook that suggests a `Time:` footer on each commit. The two sources coexist; a future Gitsweeper tool will reconcile them.
