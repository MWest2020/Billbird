...
func main() {
	...
	// HTTP server; `init` bootstraps a GitHub App via the manifest flow;
	// `doctor` runs a one-shot diagnostic against the configured Postgres
	// and GitHub App.
	if len(os.Args) > 1 {
		switch os.Args[1] {
		case "init":
		...
		default:
			...
			fmt.Fprintln(os.Stderr, "usage: billbird [serve|init|doctor]")
			os.Exit(2)
		}
	}
